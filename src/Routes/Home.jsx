        ...
        <form className="formularioBuscador">
          ...
          <select onChange={handleChangeCategoria} className='inputSearch'>
            ...
            {state.categorias.slice(1).map((categoria, index) => (
              <option className='optionCategoria' key={index} value={categoria.title}>{categoria.title}</option>
            ))}
          </select>
        ...
        {estadosNuevos.buscarPorFechas ?
          ...
          !estadosNuevos.buscar ?
            ...
            <div className="contenedorProductos">
              ...
              {!state.isFavorite ?
                <div className="botonesPaginado">
                  <button className={state.productos.length<=10 ? 'botonPaginadoAtras2' :'botonPaginadoAtras'} onClick={handlePreviousPage} disabled={currentPage === 0 }>Back</button>
                  <button className={state.productos.length<=10 ? 'botonPaginadoAtras2' :'botonPaginadoAtras'} onClick={handleNextPage} disabled={endIndex >= state.productos.length}>Next</button>
                </div>  
                :  
                                  
                <div className="botonesPaginado">
                   <button className={state.favoritos.length<=10 ? 'botonPaginadoAtras2' :'botonPaginadoAtras'} onClick={handlePreviousPage} disabled={currentPage === 0 }>Back</button>
                   <button className={state.favoritos.length<=10 ? 'botonPaginadoAtras2' :'botonPaginadoAtras'} onClick={handleNextPage} disabled={endIndex >= state.productos.length}>Next</button>
                </div>          
              }
